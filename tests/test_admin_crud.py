...
async def test_seeded_structure(client, manager_headers):
    ...
    layers = (await client.get("/api/apps/cellsens/releases/v4-4/layers", headers=manager_headers)).json()
...
async def test_app_lifecycle(client, admin_headers):
    ...
    assert res.json()["releaseCount"] == 1

    # the global pyramid is laid down for every new application
    layers = (await client.get("/api/apps/preciv-2-0/releases/initial-release/layers", headers=admin_headers)).json()
...
async def test_layer_lifecycle_and_cascade(client, admin_headers, qa_headers, tmp_path):
    res = await client.post("/api/apps/cellsens/releases/v4-4/layers", headers=admin_headers,
    ...
    (tmp_path / "cellSens" / "v4.4" / "smoke-testing.xlsx").write_bytes(build_workbook(SIMPLE))
    ...
    up = await client.post(
        "/api/apps/cellsens/releases/v4-4/snapshots", headers=qa_headers,
        json={"period": {"year": 2026, "month": 9},
              "layers": [{"layerId": "smoke-testing", "files": ["smoke-testing.xlsx"]}]})
    assert up.status_code == 200, up.text

    res = await client.patch("/api/apps/cellsens/releases/v4-4/layers/smoke-testing",
                             headers=admin_headers, json={"desc": "Quick smoke pass"})
    assert res.json()["desc"] == "Quick smoke pass"
    assert res.json()["recordCount"] == 3

    assert (await client.delete("/api/apps/cellsens/releases/v4-4/layers/smoke-testing",
                                headers=admin_headers)).status_code == 204
    layers = (await client.get("/api/apps/cellsens/releases/v4-4/layers", headers=admin_headers)).json()
    assert "smoke-testing" not in [l["id"] for l in layers]
    # cascade removed the layer's records: recreating it starts empty
    await client.post("/api/apps/cellsens/releases/v4-4/layers", headers=admin_headers,
                      json={"name": "Smoke testing"})
    body = (await client.get("/api/apps/cellsens/releases/v4-4/layers/smoke-testing/records",
                             headers=admin_headers)).json()
    assert body["total"] == 0
    await client.delete("/api/apps/cellsens/releases/v4-4/layers/smoke-testing", headers=admin_headers)
...
async def test_layer_inserted_at_requested_pyramid_position(client, admin_headers):
    ...
    for layer_id in ("component-testing", "exploratory-testing"):
        await client.delete(f"/api/apps/cellsens/releases/v4-4/layers/{layer_id}", headers=admin_headers)
    layers = (await client.get("/api/apps/cellsens/releases/v4-4/layers", headers=admin_headers)).json()
    assert [l["id"] for l in layers] == [d["layerId"] for d in DEFAULT_LAYERS]
...
async def test_unknown_ids_404(client, manager_headers):
    assert (await client.get("/api/apps/nope/releases/v4-4/layers", headers=manager_headers)).status_code == 404
    assert (await client.get("/api/apps/cellsens/releases/v4-4/layers/nope/records",
                             headers=manager_headers)).status_code == 404
    assert (await client.get("/api/apps/cellsens/releases/v4-4/layers/nope/dashboard",
                             headers=manager_headers)).status_code == 404
